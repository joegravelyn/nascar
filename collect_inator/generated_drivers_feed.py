...
def get_drivers_feed(url_header: str, year: int, series_id: int, sql_engine: Engine, load_to_sql: bool = True) -> tuple[bool, pd.DataFrame]:
   ...
   if api_result[0]:
      ...
      if load_to_sql:
         exist_df = pd
         df.to_sql("drivers_feed", con=sql_engine, if_exists="append", index=False)
